...
class UsuariosService {
  ...
  String _CARGAR = '/subir_excel_kpi/';
  String _CREAR_USUARIO = 'registro';
  String _EDITAR_USUARIO = '/users/';
  ...
  Future<Map<String, dynamic>> editarUsuario(
    ...
    try {
      final response = await http.put(
        Uri.parse('$URL$_EDITAR_USUARIO$userid'),
  ...
  Future<Map<String, dynamic>> subirExel(
      Uint8List bytes, String name, String detail, String idSede) async {
    try {
      var request =
          http.MultipartRequest('POST', Uri.parse('$URL$_CARGAR$idSede'));
      ...
      if (response.statusCode == 200) {
        // Modifico la respuesta para que sea similar a la del endpoint
        return {
          'message': 'Archivo Excel cargado. Procesando en segundo plano.',
        };
      } else {
      ...
      return {
        'detail': 'Se presentó un problema al cargar el archivo.',
      };
    }
  }
